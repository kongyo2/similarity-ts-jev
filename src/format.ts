...
function head(
  score: number,
  verdict: Pick<Verdict, "unsure" | "borderline" | "unstable">,
...
function renderFamily(family: Family, cwd: string): string[] {
  ...
  if (family.members.length === 2)
    return [`${lead}  ${location(family.members[0]!, cwd)} <-> ${location(family.members[1]!, cwd)}`];
...
export function formatPrettyReport(report: JevReport, cwd: string, options: PrettyOptions = {}): string {
  ...
  if (options.includeRejected && report.rejected.length > 0) {
    ...
    for (const pair of report.rejected) {
      lines.push(
        `${head(pair.judgment.score, pair.verdict, undefined)}  ${location(pair.left, cwd)} <-> ${location(pair.right, cwd)}`,
      );
    }
  }
  return lines.join("\n");
...
export interface JsonReport {
  ...
  families: JsonFamily[];
  rejected?: JsonPair[];
  unjudged?: (Pick<JsonPair, "mode" | "similarity" | "left" | "right" | "instances"> & {
    reason: UnjudgedPair["reason"];
    error: string;
  })[];
...
export function formatStats(
  stats: JevReport["stats"] & Partial<JudgeStats>,
  thresholds: Thresholds,
  counts: { results: number; rejected: number; unjudged: number },
): string {
  const parts = [
  ...
  const trouble = [
    ...
    stats.splits > 0 ? `${stats.splits} splits` : "",
  ].filter((part) => part !== "");
