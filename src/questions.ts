...
export function buildState(repository: string, options: StateOptions = {}): Record<string, unknown> {
  return {
    task: TASK,
    repository,
    ...(options.conventions !== undefined && options.conventions.trim() !== ""
      ? { repository_conventions: options.conventions.trim() }
      : {}),
  };
}
...
export function questionIds(index: number): QuestionIds {
  return {
    ...
    sameLogic: `p${index}_same_logic`,
    sameConcept: `p${index}_same_concept`,
...
export function pairQuestions(snippet: PairSnippet): Questions {
  ...
  return {
    ...
    [ids.sameConcept]: noul(
      { question: SAME_CONCEPT.question, a, b, ...family },
...
export function pairTokens(snippet: PairSnippet): number {
  ...
  return Object.values(questions).reduce(
    (sum, question) => sum + estimateTokens(question) + QUESTION_ENTRY_OVERHEAD,
    0,
  );
}
