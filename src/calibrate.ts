...
export function parseLabels(parsed: unknown, source: string): Labels {
  ...
  for (const [key, value] of Object.entries(parsed as Record<string, unknown>)) {
    ...
    else if (typeof value === "object" && value !== null && typeof (value as { merge?: unknown }).merge === "boolean")
      labels[key] = (value as { merge: boolean }).merge;
...
export function pairKey(pair: Pick<JudgedPair, "left" | "right">, cwd: string): string {
  const side = (location: JudgedPair["left"]) =>
    `${toRelativePath(location.filePath, cwd)}:${location.startLine}:${location.symbolName}`;
...
export interface LabelReport {
  ...
  unsureBand: {
    ...
    reported: number;
    unsure: number;
    unsurePrecision: number | null;
    surePrecision: number | null;
  };
...
function confusion(cases: Labeled[], value: (item: Labeled) => number, cutoff: number): Confusion {
  ...
  return {
    ...
    accuracy: cases.length > 0 ? (tp + tn) / cases.length : Number.NaN,
  };
}

export function fitCutoff(
  cases: Labeled[],
  value: (item: Labeled) => number,
): { cutoff: number; separable: boolean; lowestMerge: number; highestKeep: number } {
  ...
  if (lowestMerge > highestKeep)
    return {
      cutoff: Math.round(((lowestMerge + highestKeep) / 2) * 100) / 100,
      separable: true,
      lowestMerge,
      highestKeep,
    };
  ...
  return {
    ...
    wrong: cases
      ...
      .map((item) => ({
        ...
        merge: item.merge,
        score: score(item),
        confidence: item.pair.judgment.confidence,
        sameLogic: item.pair.judgment.sameLogic,
        sameConcept: item.pair.judgment.sameConcept,
      })),
  };
}
...
export function calibrate(report: JevReport, cwd: string, labels?: Labels): Calibration {
  ...
  const spreads = pairs
    .map((pair) => pair.judgment.passes?.spread)
  ...
  return {
    ...
    headroom: {
      highestBelow: below.length > 0 ? Math.max(...below) : null,
      lowestAbove: above.length > 0 ? Math.min(...above) : null,
    },
    confidence: {
      median: quantile(confidences, 0.5),
      p10: quantile(confidences, 0.1),
      p90: quantile(confidences, 0.9),
    },
    passes:
      report.stats.passes > 1
        ? {
            count: report.stats.passes,
            spreadMean: spreads.length > 0 ? mean(spreads) : null,
            spreadP90: spreads.length > 0 ? quantile(spreads, 0.9) : null,
            unstable: pairs.filter((pair) => pair.verdict.unstable).length,
          }
        : null,
...
export function formatCalibration(c: Calibration): string {
  ...
  lines.push(
    `${c.judged} judged pairs; ${c.over} at or over ${minScore.toFixed(2)}, ${c.borderline} within ${margin.toFixed(2)} of it, ${c.unsure} reported with confidence under ${unsureBelow.toFixed(2)}`,
  );
  const widest = Math.max(1, ...c.histogram.map((bin) => bin.count));
  for (const bin of c.histogram) {
    lines.push(
      `  ${bin.from.toFixed(2)}-${bin.to.toFixed(2)}  ${"#".repeat(Math.round((bin.count / widest) * 40)).padEnd(40)} ${bin.count}`,
    );
  }
  lines.push(
  ...
  lines.push(
    `headroom: highest score under the cutoff ${f2(c.headroom.highestBelow)}, lowest at or over it ${f2(c.headroom.lowestAbove)}; confidence median ${f2(c.confidence.median)} (p10 ${f2(c.confidence.p10)}, p90 ${f2(c.confidence.p90)})`,
  );
  if (c.passes !== null) {
    lines.push(
      `${c.passes.count} passes: mean spread ${f2(c.passes.spreadMean)}, p90 ${f2(c.passes.spreadP90)}; ${c.passes.unstable} pairs crossed the cutoff between passes`,
    );
  }
  if (c.labels !== null) {
    const l = c.labels;
    lines.push("");
    lines.push(
      `labels: ${l.labeled} labeled pairs found (${l.merges} merge, ${l.keeps} keep)${l.unmatched > 0 ? `, ${l.unmatched} labels matched no pair` : ""}`,
    );
    const at = l.atCutoff;
    lines.push(
      `  at ${at.cutoff.toFixed(2)}: precision ${f2(at.precision)}, recall ${f2(at.recall)}, accuracy ${f2(at.accuracy)} (tp ${at.tp}, fp ${at.fp}, fn ${at.fn}, tn ${at.tn})`,
    );
    for (const [name, s] of [
      ["score", l.signals.score],
      ["same_logic", l.signals.sameLogic],
      ["same_concept", l.signals.sameConcept],
    ] as const) {
      lines.push(
    ...
    lines.push(
      `  unsure band (confidence < ${band.unsureBelow.toFixed(2)}): ${band.unsure} of ${band.reported} reported; precision ${f2(band.unsurePrecision)} there, ${f2(band.surePrecision)} for the rest`,
    );
    for (const w of l.wrong) {
      lines.push(
        `  wrong: ${w.score.toFixed(2)} (confidence ${w.confidence.toFixed(2)}, logic ${w.sameLogic.toFixed(2)}, concept ${w.sameConcept.toFixed(2)}) labeled ${w.merge ? "merge" : "keep"}  ${w.key}`,
      );
    }
  }
  return lines.join("\n");
