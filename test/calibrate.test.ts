...
function judged(index: number, score: number, extra: Parameters<typeof judgment>[1] = {}): JudgedPair {
  ...
  return {
    ...pair(location(`/r/a${index}.ts`, 1, 5, `a${index}`), location(`/r/b${index}.ts`, 10, 15, `b${index}`)),
    judgment: j,
    verdict: decide(j),
  };
}
...
describe("calibrate", () => {
  ...
  it("finds the widest gap, ranks with AUC, and fits a cutoff in the gap when the classes separate", () => {
    ...
    assert.equal(
      best.cutoff,
      2.4,
      "the cutoff that gains the most true positives net of false positives and misses: 2 - 1 - 0 at 2.4 beats 1 - 0 - 1 at 2.6",
    );
  ...
  it("lets the fit report nothing when every labeled pair is a keep, and reports no hold-out accuracy without an evaluated fold", () => {
    ...
    assert.deepEqual(
      fit,
      { cutoff: 1.01, separable: false, lowestMerge: Number.NaN, highestKeep: 1.0 },
      "a cutoff above the highest score flags nothing, which beats flagging a keep",
    );
    const mixed = [
      ...keeps,
      { key: "c", merge: true, pair: judged(2, 2.0) },
      { key: "d", merge: false, pair: judged(3, 2.5) },
    ];
    assert.equal(
      fitCutoff(mixed, (c) => c.pair.judgment.score).cutoff,
      2.0,
      "one true positive and one false positive still beat reporting nothing",
    );
    ...
    assert.equal(
      two.accuracy,
      0.5,
      "trained on the lower keep alone, the fit sits just above it and flags the higher keep",
    );
  ...
  it("describes the distribution, the headroom, the unsure band, and the labeled accuracy", () => {
    const pairs = [
      judged(0, 2.6, { confidence: 0.8 }),
      judged(1, 2.0, { confidence: 0.2 }),
      judged(2, 1.8),
      judged(3, 0.4),
      judged(4, 1.0),
    ];
    const labels = {
      "a0.ts:1:a0 <-> b0.ts:10:b0": true,
      "a1.ts:1:a1 <-> b1.ts:10:b1": false,
      "a2.ts:1:a2 <-> b2.ts:10:b2": true,
      "a3.ts:1:a3 <-> b3.ts:10:b3": false,
      "zz <-> zz": false,
    };
    ...
    assert.equal(
      c.histogram.reduce((sum, bin) => sum + bin.count, 0),
      5,
    );
    ...
    assert.deepEqual(c.labels?.atCutoff, {
      cutoff: 1.9,
      tp: 1,
      fp: 1,
      fn: 1,
    ...
    assert.deepEqual(
      c.labels?.wrong.map((w) => w.key),
      ["a1.ts:1:a1 <-> b1.ts:10:b1", "a2.ts:1:a2 <-> b2.ts:10:b2"],
    );
  ...
  it("summarizes the passes of a repeated run", () => {
    const pairs = [
      judged(0, 2.0, { passes: { count: 3, scores: [1.8, 2.0, 2.2], spread: 0.4 } }),
      judged(1, 0.5, { passes: { count: 3, scores: [0.5, 0.5, 0.5], spread: 0 } }),
    ];
    const c = calibrate(report(pairs, 3), "/r");
    assert.deepEqual(c.passes, { count: 3, spreadMean: 0.2, spreadP90: 0.4, unstable: 1 });
    assert.match(
      formatCalibration(c),
      /3 passes: mean spread 0.20, p90 0.40; 1 pairs crossed the cutoff between passes/,
    );
  });
});
