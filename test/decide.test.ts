...
describe("decide", () => {
  it("reports at or over the cutoff, never gating on confidence", () => {
    assert.deepEqual(thresholds(), { minScore: DEFAULT_MIN_SCORE, unsureBelow: DEFAULT_UNSURE_BELOW, margin: DEFAULT_MARGIN });
    const sure = decide(judgment(2.6, { confidence: 0.9 }));
    assert.deepEqual(sure, { refactor: true, unsure: false, borderline: false, unstable: false, reason: "score>=1.90" });
  ...
  it("marks the band around the cutoff on both sides, and lets the thresholds move", () => {
    ...
    assert.equal(decide(judgment(2.0, { confidence: 0.55 }), { unsureBelow: 0.6 }).reason, "score>=1.90 (unsure, borderline)");
  });
  ...
  it("marks a pair unstable only when its passes disagree across the cutoff", () => {
    ...
    assert.equal(decide(judgment(2.4, { passes: { count: 2, scores: [1.8, 3.0], spread: 1.2 } })).reason, "score>=1.90 (unstable)");
