...
describe("SnippetReader", () => {
  it("keeps the comment block above the declaration, whatever its shape", async () => {
    ...
    for (const [text, doc] of cases) {
      ...
      const snippet = await reader(text).snippet({ filePath: "/r/a.ts", startLine: start, endLine: start, symbolName: "f", kind: "function" }, "functions");
  ...
  it("dedents the code, clips very long declarations, and extends single-line overlap windows", async () => {
    const lines = ["function outer() {", "    const a = 1;", "    const b = 2;", "  }", ...Array.from({ length: 20 }, (_, i) => `line ${i}`)];
    const text = lines.join("\n");
    const inner = await reader(text).snippet({ filePath: "/r/a.ts", startLine: 2, endLine: 3, symbolName: "a", kind: "fragment" }, "overlap");
    assert.equal(inner.code, "const a = 1;\nconst b = 2;");
    assert.equal(inner.lines, "2-3");
    const window = await reader(text).snippet({ filePath: "/r/a.ts", startLine: 5, endLine: 5, symbolName: "w", kind: "fragment" }, "overlap");
    assert.equal(window.lines, "5-17");
    const clipped = await new SnippetReader({ cwd: "/r", maxCodeChars: 100, readFile: async () => "x".repeat(400) }).snippet(
      { filePath: "/r/a.ts", startLine: 1, endLine: 1, symbolName: "x", kind: "function" },
      "functions",
    );
