...
async function runJson(args: string[]) {
  ...
  const code = await runCli([".", "--format", "json", "--cache", FIXTURE_CACHE, ...args], io, {
    client: offline,
    cwd: FIXTURE_PROJECT,
  });
...
const label = (p: { left: { symbolName: string }; right: { symbolName: string } }) =>
  `${p.left.symbolName}<->${p.right.symbolName}`;
...
describe("similarity-ts-jev CLI (replayed)", () => {
  it("prints only the pairs Jev judged worth refactoring, as data, with the change a reviewer would ask for", async () => {
    ...
    assert.deepEqual(
      ...
      ["families", "results"],
      "no stats, counts, thresholds, or bookkeeping unless asked",
    );
    ...
    assert.ok(
      report.results.every((p, i, all) => i === 0 || all[i - 1]!.score >= p.score),
      "best first",
    );
    ...
    assert.ok(
      report.families.every((f, i, all) => i === 0 || all[i - 1]!.score >= f.score),
      "best first",
    );
  });
  ...
  it("never says which detector reported a pair", async () => {
    ...
    for (const format of ["json", "pretty"]) {
      out.length = 0;
      await runCli([".", "--format", format, "--cache", FIXTURE_CACHE, "--all", "--fallow-min-tokens", "20"], io, {
        client: offline,
        cwd: FIXTURE_PROJECT,
      });
      const text = out.join("\n");
      const withoutPaths = text
        .split(FIXTURE_PROJECT)
        .join("")
        .split(FIXTURE_PROJECT.replaceAll("\\", "\\\\"))
        .join("");
      assert.doesNotMatch(
        withoutPaths,
        /similarity-ts|fallow|detectedBy|detectors|fingerprint|clone|\bvia\b/i,
        `${format} output names a detector`,
      );
    }
  });
  ...
  it("adds the rejected pairs with --all and applies the thresholds", async () => {
    ...
    assert.deepEqual(sure.report.results.filter((p) => !p.unsure).map(label), [
      "cleanEscapedString<->cleanEscapedString",
    ]);
  ...
  it("prints one line per family with the score, a flag, and the shape, and writes --output silently", async () => {
    ...
    assert.match(
      lines[0]!,
      /^2\.\d\d  copy     src\/text\/format\.ts:27-35 cleanEscapedString <-> src\/text\/lightFormat\.ts:17-25 cleanEscapedString$/,
    );
    ...
    assert.match(
      text,
      /^2\.\d\d\? extract  src\/text\/format\.ts:6-12 weekStartsOn <-> src\/text\/parse\.ts:2-8 weekStartsOn$/m,
      "unsure pairs carry ?",
    );
    assert.match(
      text,
      /^1\.9\d~ extract  src\/text\/format\.ts:12-17 part <-> src\/text\/lightFormat\.ts:6-11 part$/m,
      "borderline pairs carry ~",
    );
    assert.ok(
      lines.every((l) => /^(\d\.\d\d[ ?~!] (copy|derive|extract)( +\S.*)?|      \S.*)$/.test(l)),
      `only scores, flags, shapes, and locations:\n${text}`,
    );
    ...
    assert.match(
      all.lines[blank + 1]!,
      /^1\.\d\d~ -        src\/api\/isFriday\.ts:1-4 isFriday <-> src\/api\/isMonday\.ts:1-4 isMonday$/,
      "rejected pairs have no shape and keep their flag",
    );
    ...
    await withTempDir(async (dir) => {
      ...
      assert.equal(
        await runCli([".", "--cache", FIXTURE_CACHE, "--output", file], written.io, {
          client: offline,
          cwd: FIXTURE_PROJECT,
        }),
        0,
  ...
  it("prints nothing when no pair is worth refactoring", async () => {
    const { io, out, err } = capture();
    assert.equal(
      await runCli([".", "--cache", FIXTURE_CACHE, "--min-score", "3"], io, { client: offline, cwd: FIXTURE_PROJECT }),
      0,
    );
  ...
  it("exits 1 with --fail-on-duplicates when something is worth refactoring", async () => {
    const { io } = capture();
    assert.equal(
      await runCli([".", "--cache", FIXTURE_CACHE, "--fail-on-duplicates"], io, {
        client: offline,
        cwd: FIXTURE_PROJECT,
      }),
      1,
    ...
    assert.equal(
      await runCli([".", "--cache", FIXTURE_CACHE, "--fail-on-duplicates", "--min-score", "3"], io, {
        client: offline,
        cwd: FIXTURE_PROJECT,
      }),
      0,
  ...
  it("asks every pair again with --repeat, decides on the mean, and shows the passes", async () => {
    ...
    for (const p of report.results) {
      ...
      assert.ok(
        Math.abs(p.passes!.scores.reduce((sum, s) => sum + s, 0) / 3 - p.score) < 5e-5,
        "the score is the mean of the passes",
      );
      assert.equal(
        p.passes!.spread,
        Math.round((Math.max(...p.passes!.scores) - Math.min(...p.passes!.scores)) * 1000) / 1000,
      );
  ...
  it("prints the spend with --stats, on stderr for pretty and in the document for json", async () => {
    ...
    assert.match(
      pretty.err[0]!,
      /^6 worth refactoring, 6 left as they are; 12 pairs from 10 files; 0 requests, 1 answered from the cache; 0 input tokens \(~\$0\.0000\); \d+\.\ds; thresholds: min-score 1\.90, unsure-below 0\.50, margin 0\.25$/,
    );
    ...
    assert.match(
      repeated.err[0]!,
      /^5 worth refactoring, 7 left as they are; 12 pairs from 10 files; 0 requests, 3 answered from the cache, 3 passes;.*min-score 2\.00/,
    );
    ...
    assert.deepEqual(Object.keys(json.report.stats!).sort(), [
      "cacheHits",
      "elapsedMs",
      "fileCount",
      "inputTokens",
  ...
  it("records a run and replays it under other thresholds without detection or requests", async () => {
    await withTempDir(async (dir) => {
      ...
      const code = await runCli(["--replay", file, "--format", "json", "--all"], replayed.io, {
        client: offline,
        cwd: elsewhere,
      });
      assert.equal(code, 0, replayed.err.join("\n"));
      assert.deepEqual(
        JSON.parse(replayed.out.join("\n")),
        recorded.report,
        "the same thresholds give the same document",
      );

      const strict = capture();
      assert.equal(
        await runCli(["--replay", file, "--format", "json", "--min-score", "2.9", "--stats"], strict.io, {
          cwd: elsewhere,
        }),
        0,
      );
      ...
      assert.equal(
        await runCli(["--replay", file, "--min-score", "1.5", "--fail-on-duplicates"], pretty.io, { cwd: elsewhere }),
        1,
        "the gate applies to the replayed decisions",
      );
      const lines = pretty.out.join("\n").split("\n");
      assert.match(
        lines[0]!,
        /^2\.\d\d  copy     src\/text\/format\.ts:27-35 cleanEscapedString/,
        "paths stay relative to the recorded project",
      );
      assert.ok(
        lines.some((l) => /isFriday/.test(l)),
        "a lower cutoff reports more",
      );
      ...
      assert.equal(
        await runCli(["--replay", tuned, "--format", "json", "--stats"], asRecorded.io, { cwd: elsewhere }),
        0,
      );
      const asRecordedReport = JSON.parse(asRecorded.out.join("\n")) as JsonReport;
      assert.deepEqual(
        asRecordedReport.thresholds,
        { minScore: 2.5, unsureBelow: 0.5, margin: 0.1 },
        "a replay without threshold flags keeps the recorded thresholds",
      );
      assert.ok(asRecordedReport.results.length > 0 && asRecordedReport.results.every((p) => p.score >= 2.5));
      const overridden = capture();
      assert.equal(
        await runCli(["--replay", tuned, "--format", "json", "--stats", "--min-score", "1.9"], overridden.io, {
          cwd: elsewhere,
        }),
        0,
      );
      assert.deepEqual(
        (JSON.parse(overridden.out.join("\n")) as JsonReport).thresholds,
        { minScore: 1.9, unsureBelow: 0.5, margin: 0.1 },
  ...
  it("describes the score distribution with --calibrate, and the accuracy against --labels", async () => {
    ...
    assert.equal(
      lines[0],
      "12 judged pairs; 6 at or over 1.90, 2 within 0.25 of it, 1 reported with confidence under 0.50",
    );
    ...
    assert.match(
      text,
      /^headroom: highest score under the cutoff 1\.8\d, lowest at or over it 1\.9\d; confidence median \d\.\d\d \(p10 \d\.\d\d, p90 \d\.\d\d\)$/m,
    );
    assert.doesNotMatch(text, /labels:|passes:/);
    const repeated = await runPretty(["--calibrate", "--repeat", "3"]);
    assert.match(
      repeated.text,
      /^3 passes: mean spread \d\.\d\d, p90 \d\.\d\d; \d+ pairs crossed the cutoff between passes$/m,
    );
    const withStats = await runPretty(["--calibrate", "--stats"]);
    assert.equal(withStats.err.length, 1);
    assert.match(
      withStats.err[0]!,
      /^6 worth refactoring, 6 left as they are; 12 pairs from 10 files; 0 requests, 1 answered from the cache; /,
    );
    const jsonWithStats = capture();
    assert.equal(
      await runCli([".", "--cache", FIXTURE_CACHE, "--calibrate", "--stats", "--format", "json"], jsonWithStats.io, {
        client: offline,
        cwd: FIXTURE_PROJECT,
      }),
      0,
    ...
    await withTempDir(async (dir) => {
      ...
      assert.match(
        labeled.text,
        /^  at 1\.90: precision 0\.75, recall 1\.00, accuracy 0\.83 \(tp 3, fp 1, fn 0, tn 2\)$/m,
      );
      assert.match(
        labeled.text,
        /^  score {8}AUC \d\.\d\d; fitted cutoff \d\.\d\d \((not )?separable: lowest merge \d\.\d\d, highest keep \d\.\d\d\); at the fit precision \d\.\d\d recall \d\.\d\d; \d-fold hold-out accuracy \d\.\d\d/m,
      );
      assert.match(
        labeled.text,
        /^  unsure band \(confidence < 0\.50\): 1 of 4 reported; precision 1\.00 there, 0\.67 for the rest$/m,
      );
      assert.match(
        labeled.text,
        /^  wrong: 1\.9\d \(confidence \d\.\d\d, logic \d\.\d\d, concept \d\.\d\d\) labeled keep  src\/text\/format\.ts:12:part <-> src\/text\/lightFormat\.ts:6:part$/m,
      );

      const { io, out } = capture();
      assert.equal(
        await runCli([".", "--cache", FIXTURE_CACHE, "--calibrate", "--labels", file, "--format", "json"], io, {
          client: offline,
          cwd: FIXTURE_PROJECT,
        }),
        0,
      ...
      assert.deepEqual(calibration.labels?.atCutoff, {
        cutoff: 1.9,
        tp: 3,
        fp: 1,
        fn: 0,
      ...
      const refused = capture();
      assert.equal(
        await runCli([".", "--cache", FIXTURE_CACHE, "--calibrate", "--labels", bad], refused.io, {
          client: offline,
          cwd: FIXTURE_PROJECT,
        }),
        1,
  ...
  it("sends --conventions with every request and asks again when they change", async () => {
    ...
    const code = await runCli(
      [".", "--conventions", "Locale files stay separate on purpose.", "--format", "json", "--stats"],
      io,
      { client, cwd: FIXTURE_PROJECT },
    );
    assert.equal(code, 0, err.join("\n"));
  ...
  it("counts without asking Jev in --dry-run, honoring --max-pairs, --repeat, --pairs-per-request, and the warning gate", async () => {
    ...
    out.length = 0;
    assert.equal(
      await runCli([".", "--dry-run", "--max-pairs", "3"], io, { client: offline, cwd: FIXTURE_PROJECT }),
      0,
    );
    assert.match(out.join("\n"), /^3 pairs, 1 requests, \d+ tokens, ~\$0\.\d{4}$/);
    out.length = 0;
    assert.equal(
      await runCli([".", "--dry-run", "--repeat", "3", "--conventions", "Locale files stay separate."], io, {
        client: offline,
        cwd: FIXTURE_PROJECT,
      }),
      0,
    ...
    out.length = 0;
    assert.equal(
      await runCli([".", "--dry-run", "--pairs-per-request", "5"], io, { client: offline, cwd: FIXTURE_PROJECT }),
      0,
    );
    assert.match(out.join("\n"), /^12 pairs, 3 requests, /);
    out.length = 0;
    ...
    assert.equal(
      await runCli(["src", "missing-dir", "--dry-run", "--fail-on-warnings"], io, {
        client: offline,
        cwd: FIXTURE_PROJECT,
      }),
      1,
    ...
    out.length = 0;
    assert.equal(
      await runCli([".", "--dry-run", "--same-file-only"], io, { client: offline, cwd: FIXTURE_PROJECT }),
      0,
    );
    assert.match(
      out.join("\n"),
      /^0 pairs, 0 requests, 0 tokens, ~\$0\.0000$/,
      "every duplicate in the fixture spans two files",
    );
  });
  ...
  it("writes an empty file for an empty report", async () => {
    await withTempDir(async (dir) => {
      ...
      assert.equal(
        await runCli([".", "--cache", FIXTURE_CACHE, "--min-score", "3", "--output", file], io, {
          client: offline,
          cwd: FIXTURE_PROJECT,
        }),
        0,
  ...
  it("maps analysis problems to 1 and judgment failures to 2", () => {
    const base = {
      ...
      rejectedCount: 0,
      unjudged: [],
      thresholds: { minScore: 1.9 },
    } as unknown as JevReport;
    const pair = {
      mode: "functions",
      similarity: 1,
      left: { filePath: "a", startLine: 1, endLine: 1, symbolName: "a", kind: "function" },
      right: { filePath: "b", startLine: 1, endLine: 1, symbolName: "b", kind: "function" },
    } as const;
    ...
    assert.equal(
      exitCode(
        { ...base, stats, unjudged: [{ ...pair, reason: "capped", error: "over the --max-pairs limit (1)" }] },
        gates,
      ),
      0,
    );
    assert.equal(
      exitCode(
        { ...base, stats, unjudged: [{ ...pair, reason: "unreadable", error: "could not read the source: ENOENT" }] },
        gates,
      ),
      1,
    );
    assert.equal(
      exitCode({ ...base, stats, unjudged: [{ ...pair, reason: "api", error: "InternalServerError: 503" }] }, gates),
      2,
    );
    ...
    assert.equal(
      exitCode({ ...base, stats: { fileCount: 0 } as JevReport["stats"], warnings: [{ message: "w" }] }, gates),
      1,
    );
  });
  ...
  it("exits 2 and lists the pairs it could not judge when Jev fails", async () => {
    ...
    const { io, out, err } = capture();
    const code = await runCli([".", "--format", "json", "--retries", "0"], io, {
      client: failing,
      cwd: FIXTURE_PROJECT,
    });
    assert.equal(code, 2);
  ...
  it("rejects bad options, unknown modes, and a run with neither paths nor --replay", async () => {
    ...
    assert.equal(
      await runCli([".", "--same-file-only", "--cross-file-only"], io, { client: offline, cwd: FIXTURE_PROJECT }),
      1,
    );
    err.length = 0;
